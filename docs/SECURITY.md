...
Use a GitHub App installation token for production. Observation needs repository metadata and Actions read; retries additionally need Actions write; issue creation needs Issues write; merge needs pull-requests write (and contents as required by merge method). Monitoring needs no write scopes. Administration is never required. Fine-grained tokens should be restricted to opted-in repositories and these exact permissions. Do not reuse an operator admin token in the deployed service. Secrets are injected by the fleet credential broker or the deployment secret provider, never committed or printed.
...
## Roles
Observer: read workflow metadata and export/report. Retry executor: explicit repository/workflow allowlist and `dry_run:false`, maximum three retry reservations per incident; failures/unknown submissions consume budget. Operator: reviewed configuration and deployment. Merge role requires Contents/Pull requests write plus explicit `enable_merge` and allowlists; revert/update roles are not implemented. Database filesystem ACLs must isolate the service identity from untrusted agents. Separate OS/deployment identities enforce these roles; this CLI is not a multi-user authentication server.
...
Set `dry_run:true` or revoke the executor token to prevent new retries. Stop the dedicated babysitter deployment, not the product fleet. Preserve the ledger and outstanding reservations. Roll back code by reviewed release revision; do not reset the ledger or retry budget. Reconcile uncertain actions before re-enabling. Revert/dependency-update remain unimplemented. Merge/issue paths stay dry-run and flag-disabled until operator-approved rollout.
